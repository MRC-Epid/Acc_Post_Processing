...
if __name__ == '__main__':
 ...
 text_no_files="No observations to summarize")
        ...
        check_negative_values(df=summary_df, log=verif_log, text_to_log="There are negative values in the enmo_*plus variables.", description="Check to see if device has calibrated correctly. \n If the negative enmo value is close to 0 it could be a rounding issue. \n It is suggested to remove data/file if any negative values are present. Negative values can be removed from the daily and summary level release files by setting the REMOVE_NEGATIVE_VALUES switch to yes in the config log. NOTE, this should only be done once the values have been investigated.", variables= enmo_variables, text_no_error="There are no files with negative values in any of the enmo_variables. No files to check.")
