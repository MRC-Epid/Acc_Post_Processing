...
def merging_data(file_id, metadata_df, datafile_df, anomalies_df):
...
def pwear_variables(valid_df, time_resolution):
...
def mechanical_noise(formatted_df):
    ...
    dataframe = formatted_df.copy()

    return dataframe


# OUTPUTTING THE DATAFRAME TO THE INDIVIDUAL_PARTPRO_FILES FOLDER
def outputting_dataframe(dataframe, file_id):
    ...
    file_name = os.path.join(file_path, f"{file_id}_{config.OUTPUT_FILE_EXT}.csv")
...
if __name__ == '__main__':
    ...
    for file_id in files_list:
        metadata_df = reading_metadata(file_id)
        datafile_df = reading_datafile(file_id)
        if config.PROCESSING.lower() == 'pampro':
            anomalies_df = anomalies()
            time_resolution, merged_df = merging_data(file_id, metadata_df, datafile_df, anomalies_df)

        if config.PROCESSING.lower() == 'wave':
            time_resolution, merged_df = merging_data(file_id, metadata_df, datafile_df, None)
        valid_df = indicator_variable(time_resolution, merged_df)
        formatted_df = pwear_variables(valid_df, time_resolution)
        if config.USE_WEAR_LOG == 'Yes':
            wear_log(formatted_df)
        dataframe = mechanical_noise(formatted_df)
        outputting_dataframe(dataframe, file_id)
